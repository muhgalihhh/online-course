                    ...
                    <div className="mt-6 flex-1">
                        {!isAuthenticated ? (
                            <div className="flex flex-col items-center justify-center py-8 text-center">
                                ...
                                <p className="mb-4 text-muted-foreground">Anda harus login terlebih dahulu untuk melihat keranjang</p>
                                <Button
                                    variant="default"
                                    onClick={() => {
                                        setIsOpen(false);
                            ...
                            <div className="flex flex-col items-center justify-center py-8 text-center">
                                ...
                                <p className="text-muted-foreground">Belum ada kursus dalam keranjang</p>
                                <Button
                                    variant="outline"
                                    className="mt-4"
                                    onClick={() => {
                            ...
                            <ScrollArea className="h-[calc(100vh-280px)]">
                                            ...
                                            {pendingItems.map((item) => (
                                                <div key={item.id} className="flex gap-3 rounded-lg border border-yellow-200 bg-yellow-50/50 p-3">
                                                    {item.thumbnail ? (
                                                        ...
                                                        <div className="flex h-16 w-20 items-center justify-center rounded bg-gray-200">
                                                            <ShoppingCart className="h-6 w-6 text-gray-400" />
                                                        </div>
                                                    )}
                                                    <div className="min-w-0 flex-1">
                                                        <h4 className="truncate text-sm font-medium">{item.title}</h4>
                                                        <p className="text-sm font-semibold text-yellow-700">{formatPrice(item.price)}</p>
                                ...
                                {completedItems.length > 0 && (
                                    <div className="mb-6">
                                        <h3 className="mb-3 text-sm font-semibold text-green-700">Pembayaran Berhasil ({completedItems.length})</h3>
                                        <div className="space-y-3">
                                            {completedItems.map((item) => (
                                                <div key={item.id} className="flex gap-3 rounded-lg border border-green-200 bg-green-50/50 p-3">
                                                    {item.thumbnail ? (
                                                        <img src={item.thumbnail} alt={item.title} className="h-16 w-20 rounded object-cover" />
                                                    ) : (
                                                        <div className="flex h-16 w-20 items-center justify-center rounded bg-gray-200">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <h4 className="truncate text-sm font-medium">{item.title}</h4>
                                                        <p className="text-sm font-semibold text-green-700">{formatPrice(item.price)}</p>
                                                        <div className="mt-1 flex items-center gap-1">
                                                            {getStatusIcon(item.status)}
                                ...
                                {failedItems.length > 0 && (
                                    <div className="mb-6">
                                        <h3 className="mb-3 text-sm font-semibold text-red-700">Pembayaran Gagal/Expired ({failedItems.length})</h3>
                                        <div className="space-y-3">
                                            {failedItems.map((item) => (
                                                <div key={item.id} className="flex gap-3 rounded-lg border border-red-200 bg-red-50/50 p-3">
                                                    {item.thumbnail ? (
                                                        <img
                                                            src={item.thumbnail}
                                                            alt={item.title}
                                                            className="h-16 w-20 rounded object-cover opacity-50"
                                                        />
                                                    ) : (
                                                        <div className="flex h-16 w-20 items-center justify-center rounded bg-gray-200">
                                                            <ShoppingCart className="h-6 w-6 text-gray-400" />
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <h4 className="truncate text-sm font-medium text-gray-500 line-through">{item.title}</h4>
                        ...
                        <SheetFooter className="border-t pt-4">
                            <div className="w-full space-y-3">
                                <div className="flex items-center justify-between">
                                    <span className="text-sm text-muted-foreground">Total Belum Dibayar:</span>
                                    <span className="text-lg font-bold text-yellow-700">{formatPrice(getTotalAmount())}</span>
                                </div>
                                <Button className="w-full" onClick={() => loadTransactions()} disabled={isLoading}>
                                    {isLoading ? (
                                        <>
            ...
            <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Apakah Anda yakin ingin membatalkan transaksi untuk kursus "{itemToDelete?.title}"? Tindakan ini tidak dapat dibatalkan.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel disabled={isDeleting}>Tidak, Pertahankan</AlertDialogCancel>
                        <AlertDialogAction
