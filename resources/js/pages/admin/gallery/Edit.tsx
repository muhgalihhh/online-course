...
interface GalleryEditProps extends PageProps {
    gallery: Gallery;
}

export default function GalleryEdit({ gallery }: GalleryEditProps) {
    const [preview, setPreview] = useState<string | null>(null);
    const [fileType, setFileType] = useState<'image' | 'video'>(gallery.file_type);
    const [videoSource, setVideoSource] = useState<'file' | 'youtube'>(gallery.video_source || 'file');
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        video_source: gallery.video_source || ('file' as 'file' | 'youtube'),
    ...
    // Set initial preview
    useEffect(() => {
        if (gallery.file_type === 'video' && gallery.video_source === 'youtube') {
            if (gallery.youtube_thumbnail) {
                setPreview(gallery.youtube_thumbnail);
            } else if (gallery.youtube_url) {
                const videoId = extractYouTubeVideoId(gallery.youtube_url);
                if (videoId) {
                    setPreview(`https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`);
                }
            }
        } else if (gallery.file_url) {
            setPreview(gallery.file_url);
        }
    }, []);
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p className="text-muted-foreground">Ubah informasi item galeri</p>
                        </div>
                    </div>
                </div>
                        ...
                        <Card className="lg:col-span-1">
                            ...
                            <CardContent className="space-y-4">
                                {data.file_type === 'video' && videoSource === 'youtube' ? (
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="file">File {data.file_type === 'image' ? 'Gambar' : 'Video'} (Opsional)</Label>
                                                ...
                                                {preview ? (
                                                    <div className="relative h-full w-full">
                                                        ...
                                                        <div className="bg-opacity-50 absolute top-2 right-2 rounded bg-black px-2 py-1 text-xs text-white">
                                                            {data.file ? 'File Baru' : 'File Saat Ini'}
                                                        </div>
                                                    </div>
                                                ) : (
                                                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                                        ...
                                                        <p className="mb-2 text-sm text-gray-500">
                                                            <span className="font-semibold">Klik untuk upload file baru</span> atau drag and drop
                                                        </p>
                                                        <p className="text-xs text-gray-500">
                                        ...
                                        {data.file && (
                                            <div className="text-sm text-gray-600">
                                                <p>File baru dipilih: {data.file.name}</p>
