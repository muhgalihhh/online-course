...
import { BookOpen, ChevronRight, Filter, Plus, Search } from 'lucide-react';
...
export default function Chapters({ courses, categories, institutions }: ChaptersProps) {
    ...
    const filteredCourses = courses.data.filter((course) => {
        ...
        const matchesCategory = categoryFilter === 'all' || (course.category && course.category.id.toString() === categoryFilter);

        const matchesInstitution = institutionFilter === 'all' || (course.institution && course.institution.id.toString() === institutionFilter);

        const matchesType = typeFilter === 'all' || (typeFilter === 'pro' && course.is_pro) || (typeFilter === 'free' && !course.is_pro);
                ...
                {paginatedCourses.map((course) => (
                    <Card key={course.id} className="overflow-hidden transition-shadow hover:shadow-md">
                        ...
                        <CardFooter className="pt-2 pb-3">
                            <Link href={route('admin.chapters.by-course', course.id)} className="flex-1">
                                <Button className="h-8 w-full text-sm" variant="default">
                    ...
                    <CardContent className="py-3">
                        <div className="flex items-center justify-between">
                            <p className="text-sm text-muted-foreground">
                                Menampilkan {(currentPage - 1) * itemsPerPage + 1} - {Math.min(currentPage * itemsPerPage, filteredCourses.length)}{' '}
            ...
            {filteredCourses.length === 0 && (
                <Card>
                    <CardContent className="flex flex-col items-center justify-center py-12">
                        ...
                        <p className="mb-4 text-center text-sm text-muted-foreground">Tidak ada kursus yang sesuai dengan filter yang Anda pilih.</p>
