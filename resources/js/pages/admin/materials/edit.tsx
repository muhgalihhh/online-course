...
import { ArrowLeft, FileText, Save, Video } from 'lucide-react';
...
export default function EditMaterial({ material, chapters }: EditMaterialProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        console.log('Edit form submission started', { data, material });
        ...
        if (data.type === 'video_youtube' && !data.youtube_url.trim()) {
            alert('Please enter a YouTube URL');
            return;
        }

        console.log('Edit validation passed, submitting to:', route('admin.materials.update', material.id));
        console.log('Final edit form data:', data);

        post(route('admin.materials.update', material.id), {
            onStart: () => console.log('Edit request started'),
            onProgress: (progress) => console.log('Edit progress:', progress),
            onSuccess: (response) => console.log('Edit success:', response),
            onError: (errors) => {
                console.error('Edit submission errors:', errors);
                alert('Error: ' + JSON.stringify(errors));
            },
            onFinish: () => console.log('Edit request finished'),
        });
    };
    ...
    const courseGroups = chapters.reduce(
        (acc, chapter) => {
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Link
                    href={materialCourseId ? route('admin.materials.index') + '?selected_course=' + materialCourseId : route('admin.materials.index')}
                >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <CardContent className="space-y-4">
                        ...
                        {material.type !== 'video_youtube' && material.file_path && (
                            <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
                                <h4 className="mb-2 font-medium text-blue-900">File Saat Ini:</h4>
                                <p className="text-sm text-blue-700">{material.file_path}</p>
                            </div>
                        )}

                        {material.type === 'video_youtube' && material.youtube_url && (
                            <div className="rounded-lg border border-red-200 bg-red-50 p-4">
                                <h4 className="mb-2 font-medium text-red-900">URL YouTube Saat Ini:</h4>
                                <p className="text-sm text-red-700">{material.youtube_url}</p>
                            </div>
                        )}
                        ...
                        {(data.type === 'pdf' || data.type === 'image' || data.type === 'video_local') && (
                            <div className="space-y-2">
                                ...
                                <Input type="file" accept={getFileAccept()} onChange={(e) => setData('file_path', e.target.files?.[0] || null)} />
                ...
                <div className="flex items-center justify-end gap-4">
                    <Link
                        href={
                            materialCourseId
                                ? route('admin.materials.index') + '?selected_course=' + materialCourseId
