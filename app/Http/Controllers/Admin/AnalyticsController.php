...
class AnalyticsController extends Controller
{
    public function index(Request $request)
    {
        ...
        $topCourses = Course::query()
            ->withCount([
                'users as enrollments_count' => function ($q) use ($dateFrom, $manualDateFrom, $manualDateTo) {
                    if ($dateFrom) {
        ...
        $topCourses->each(function ($course) {
            $course->avg_rating = (float) ($course->avg_rating ?? 0);
        });
        ...
        $topCategories = \App\Models\Category::withCount([
            'courses' => function ($query) use ($dateFrom, $manualDateFrom, $manualDateTo) {
